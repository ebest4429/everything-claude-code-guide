...
[docx.py]
====================
[역할] Word 문서(.docx) XML 파일을 XSD 스키마에 대해 검증하는 검증자입니다.

[주요 기능]
  - XSD 스키마 검증
  - 공백 보존(xml:space="preserve") 검증
  - 삭제 마킹(w:del) 내 w:t 요소 검증
  - 삽입 마킹(w:ins) 내 w:delText 요소 검증
  - 단락 수 비교 (원본 vs 수정본)
  - paraId/durableId 값 범위 제약 검증
  - 댓글 마커 쌍 검증
  - durableId 자동 수복

[주요 클래스]
  - DOCXSchemaValidator: Word 문서 검증자 (BaseSchemaValidator 상속)
"""
