...
[base.py]
====================
[역할] Office 문서 파일의 공통 검증 로직을 담은 기본 검증자 클래스입니다.

[주요 기능]
  - XML 형식 검증 (well-formedness)
  - 네임스페이스 선언 검증
  - 고유 ID 검증 (파일 내 및 전역 범위)
  - 파일 참조 및 관계(relationship) 검증
  - 콘텐츠 타입 선언 검증
  - XSD 스키마 검증 (원본 파일 오류는 무시, 신규 오류만 검출)
  - 공백 보존(xml:space="preserve") 자동 수복

[주요 클래스]
  - BaseSchemaValidator: 모든 파일 유형 검증자의 기본 클래스
"""
