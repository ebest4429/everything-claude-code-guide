...
[docx.py]
====================
[역할] Word 문서 XML 파일을 XSD 스키마 기준으로 유효성 검사합니다.

[주요 기능]
  - XSD 스키마 검사 (BaseSchemaValidator 상속)
  - 공백 보존(xml:space) 검사
  - 삭제(w:del) 및 삽입(w:ins) 요소 검사
  - 단락 수 비교
  - ID 제약 조건(paraId/durableId) 검사
  - 주석 마커 쌍 검사
  - durableId 자동 복구

[주요 클래스]
  - DOCXSchemaValidator: Word 문서 전용 검사기
"""
...
class DOCXSchemaValidator(BaseSchemaValidator):
    ...
    def validate_whitespace_preservation(self):
        ...
        if errors:
            print(f"실패 - 공백 보존 위반 {len(errors)}건 발견:")
            for error in errors:
                print(error)
            return False
        else:
            if self.verbose:
                print("통과 - 모든 공백이 올바르게 보존됨")
            return True
    ...
    def validate_deletions(self):
        ...
        if errors:
            print(f"실패 - 삭제(w:del) 유효성 위반 {len(errors)}건 발견:")
            for error in errors:
                print(error)
            return False
        else:
            if self.verbose:
                print("통과 - w:del 내부에 w:t 요소 없음")
            return True
    ...
    def validate_insertions(self):
        ...
        if errors:
            print(f"실패 - 삽입(w:ins) 유효성 위반 {len(errors)}건 발견:")
            for error in errors:
                print(error)
            return False
        else:
            if self.verbose:
                print("통과 - w:ins 내부에 w:delText 요소 없음")
            return True
    ...
    def compare_paragraph_counts(self):
        ...
        print(f"\n단락 수: {original_count} → {new_count} ({diff_str})")
    ...
    def validate_id_constraints(self):
        ...
        if errors:
            print(f"실패 - ID 제약 조건 위반 {len(errors)}건:")
            for e in errors:
                print(e)
        elif self.verbose:
            print("통과 - 모든 paraId/durableId 값이 제약 범위 내")
        return not errors
    ...
    def validate_comment_markers(self):
        ...
        if not document_xml:
            if self.verbose:
                print("통과 - document.xml 없음 (주석 검사 건너뜀)")
            return True
        ...
        if errors:
            print(f"실패 - 주석 마커 위반 {len(errors)}건:")
            for error in errors:
                print(error)
            return False
        else:
            if self.verbose:
                print("통과 - 모든 주석 마커가 올바르게 쌍을 이룸")
            return True
            ...
            try:
                ...
                for elem in dom.getElementsByTagName("*"):
                    ...
                    if needs_repair:
                        ...
                        print(
                            f"  복구: {xml_file.name}: durableId {durable_id} → {new_id}"
                        )
                        repairs += 1
                        modified = True
