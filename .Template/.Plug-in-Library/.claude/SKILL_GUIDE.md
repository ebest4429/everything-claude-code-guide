...
| 스킬명 | 호출 형태 | 설명 | 전역 | 프로젝트 |
|--------|----------|------|:----:|:-------:|
| `재정분석설계` | `/재정분석설계 <지자체명>` | 지자체 중기지방재정계획 분석 프로젝트 파일 7개 자동 생성 | ❌ | ✅ |
...
| 재정분석설계 | — | ❌ | 지자체 재정분석 전문 도구, 특정 업무 도메인 전용 |
...
| 위치 | 등록된 스킬 |
|------|-----------|
| `commands/` | translate-plugin, road-name-merge, road-name-merge-code, 도로명통합, 도로명통합-코드, 도로명편집병합-코드, skill-creator, schedule, analyze, build-dashboard, create-viz, explore-data, validate, write-query, 재정분석설계 |
