...
### Step 6 — 라이브러리 점검 (SKILL_GUIDE.md)

`SKILL_GUIDE.md`를 읽고 이 프로젝트에서 필요한 스킬/훅/에이전트를 확인:

- 기존 라이브러리에서 활용 가능한 것 파악
- 새로 만들어야 할 것 파악 → Phase 플랜에 항목 추가
- 새로 만든 후 SKILL_GUIDE.md에 추가
...
- **새 Phase 시작 시**: 기존 파일 이름 변경 → 새 파일 작성
  - 예) `WORKSPACE.md` → `WORKSPACE.Phase1.md`
...
- [ ] Phase 1 플랜 — `## 작업 항목` 섹션 포함하여 신규 작성
- [ ] 이전 마스터플랜 파일 — 삭제
...
- [ ] SKILL_GUIDE.md — 이 프로젝트에서 활용할 항목 확인 완료
